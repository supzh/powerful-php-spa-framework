...
class YaHeiTz
{
	...
	public static function  GetCpuPercentages($stat1, $stat2) {
		...
		$cpus=array();for( $i = 0, $l = count($stat1); $i < $l; $i++) {	$dif = array();	$dif['user'] = $stat2[$i]['user'] - $stat1[$i]['user'];$dif['nice'] = $stat2[$i]['nice'] - $stat1[$i]['nice'];	$dif['sys'] = $stat2[$i]['sys'] - $stat1[$i]['sys'];$dif['idle'] = $stat2[$i]['idle'] - $stat1[$i]['idle'];$dif['iowait'] = $stat2[$i]['iowait'] - $stat1[$i]['iowait'];$dif['irq'] = $stat2[$i]['irq'] - $stat1[$i]['irq'];$dif['softirq'] = $stat2[$i]['softirq'] - $stat1[$i]['softirq'];$total = array_sum($dif);$cpu = array();foreach($dif as $x=>$y) $cpu[$x] = round($y / $total * 100, 2);$cpus['cpu' . $i] = $cpu;}
		return $cpus;
	}
		
	
	public static function  makeImageUrl($title, $data) {
		$api='http://api.yahei.net/tz/cpu_show.php?id=';$url.=$data['user'].',';$url.=$data['nice'].',';$url.=$data['sys'].',';$url.=$data['idle'].',';$url.=$data['iowait'];$url.='&chdl=User|Nice|Sys|Idle|Iowait&chdlp=b&chl=';$url.=$data['user'].'%25|';$url.=$data['nice'].'%25|';$url.=$data['sys'].'%25|';$url.=$data['idle'].'%25|';$url.=$data['iowait'].'%25';$url.='&chtt=Core+'.$title;return $api.base64_encode($url);
	}
	...
	public static function  sys_freebsd()
	{
		...
		if (false === ($res['cpu']['num'] = static::get_key("hw.ncpu"))) return false;

		$res['cpu']['model'] = static::get_key("hw.model");

		//LOAD AVG

		if (false === ($res['loadAvg'] = static::get_key("vm.loadavg"))) return false;

		//UPTIME

		if (false === ($buf = static::get_key("kern.boottime"))) return false;
		...
		if (false === ($buf = static::get_key("hw.physmem"))) return false;

		$res['memTotal'] = round($buf/1024/1024, 2);


		$str = static::get_key("vm.vmtotal");
	...
	public static function  get_key($keyName)
	{
		...
		return static::do_command('sysctl', "-n $keyName");
	...
	public static function  do_command($commandName, $args)
	{
		...
		if (false === ($command = static::find_command($commandName))) return false;
	...
	public static function  sys_windows()
	{
		...
		$cpuinfo = static::GetWMI($wmi,"Win32_Processor", array("Name","L2CacheSize","NumberOfCores"));
		...
		$sysinfo = static::GetWMI($wmi,"Win32_OperatingSystem", array('LastBootUpTime','TotalVisibleMemorySize','FreePhysicalMemory','Caption','CSDVersion','SerialNumber','InstallDate'));
		...
		$swapinfo = static::GetWMI($wmi,"Win32_PageFileUsage", array('AllocatedBaseSize','CurrentUsage'));


		// LoadPercentage

		$loadinfo = static::GetWMI($wmi,"Win32_Processor", array("LoadPercentage"));
	...
	{
		$stat1 = static::GetCoreInformation();sleep(1);
		$stat2 = static::GetCoreInformation();
		$data = static::GetCpuPercentages($stat1, $stat2);
		...
		switch(PHP_OS)
		{
			...
			case "Linux":
				...
				$sysReShow = (false !== ($sysInfo = static::sys_linux()))?"show":"none";
			...
			case "FreeBSD":
				...
				$sysReShow = (false !== ($sysInfo = static::sys_freebsd()))?"show":"none";
		...
		for ($i = 2; $i < count($strs); $i++ )
		{
			...
			$NetInput[$i] = humanReadSize($info[2][0]);
			$NetOut[$i]  = humanReadSize($info[10][0]);
		}
		
		$arr=array('cpu'=>$cpu_show, 'useSpace'=>"$du",'freeSpace'=>"$df",'hdPercent'=>"$hdPercent",'barhdPercent'=>"$hdPercent%",'TotalMemory'=>"$mt",'UsedMemory'=>"$mu",'FreeMemory'=>"$mf",'CachedMemory'=>"$mc",'Buffers'=>"$mb",'TotalSwap'=>"$st",'swapUsed'=>"$su",'swapFree'=>"$sf",'loadAvg'=>"$load",'uptime'=>"$uptime",'freetime'=>"$freetime",'bjtime'=>"$bjtime",'stime'=>"$stime",'memRealPercent'=>"$memRealPercent",'memRealUsed'=>"$memRealUsed",'memRealFree'=>"$memRealFree",'memPercent'=>"$memPercent%",'memCachedPercent'=>"$memCachedPercent",'barmemCachedPercent'=>"$memCachedPercent%",'swapPercent'=>"$swapPercent",'barmemRealPercent'=>"$memRealPercent%",'barswapPercent'=>"$swapPercent%",
		
		
		'NetOut2'=>"$NetOut[2]",'NetOut3'=>"$NetOut[3]",'NetOut4'=>"$NetOut[4]",'NetOut5'=>"$NetOut[5]",'NetOut6'=>"$NetOut[6]",'NetOut7'=>"$NetOut[7]",'NetOut8'=>"$NetOut[8]",'NetOut9'=>"$NetOut[9]",'NetOut10'=>"$NetOut[10]",
		'NetInput2'=>"$NetInput[2]",'NetInput3'=>"$NetInput[3]",'NetInput4'=>"$NetInput[4]",'NetInput5'=>"$NetInput[5]",'NetInput6'=>"$NetInput[6]",'NetInput7'=>"$NetInput[7]",'NetInput8'=>"$NetInput[8]",'NetInput9'=>"$NetInput[9]",'NetInput10'=>"$NetInput[10]",
		
		'NetOutSpeed2'=> humanReadSize($NetOutSpeed[2]),'NetOutSpeed3'=>humanReadSize($NetOutSpeed[3]),'NetOutSpeed4'=>humanReadSize($NetOutSpeed[4]),'NetOutSpeed5'=>humanReadSize($NetOutSpeed[5]),'NetInputSpeed2'=>humanReadSize($NetInputSpeed[2]),'NetInputSpeed3'=>humanReadSize($NetInputSpeed[3]),'NetInputSpeed4'=>humanReadSize($NetInputSpeed[4]),'NetInputSpeed5'=>humanReadSize($NetInputSpeed[5]));
